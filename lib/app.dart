...
class _HabitFlowAppState extends State<HabitFlowApp> {
  ...
  /// whether loading habits should schedule anything at all. The trackers have
  /// no such dependency, so they load alongside rather than after.
  Future<void> _bootstrap() async {
    await _settings.load();
    await _habits.setRemindersEnabled(_settings.settings.remindersEnabled);
    await Future.wait(<Future<void>>[_habits.load(), _trackers.load()]);
  }
...
class _StartupGate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (habits.isLoading || settings.isLoading || trackers.isLoading) {
